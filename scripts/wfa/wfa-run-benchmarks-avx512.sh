...
# Replace AVX512 with 1 and AVX2 with 1
find wavefront/ -type f -exec sed -i 's/__AVX512CD__/1/g' {} \;
find wavefront/ -type f -exec sed -i 's/__AVX512VL__/1/g' {} \;
...
if [ ! -f "WFA2-lib/bin/align_benchmark" ]; then
    echo "[!] Compilation failed. Make sure your machine supports AVX512."
...
for dataset in $DATASETS_PATH/*.seq; do
    ...
    ./WFA2-lib/bin/align_benchmark -a gap-affine2p-wfa -i $dataset --wfa-memory ultralow -o $RESULTS_PATH/$(basename $dataset).wfa.avx512.out > $RESULTS_PATH/$(basename $dataset).wfa.avx512.stdout 2> $RESULTS_PATH/$(basename $dataset).wfa.avx512.stderr
    # Collecting the performance data
    perf stat -a -e instructions,cycles,cache-misses -M TopdownL1 -o $RESULTS_PATH/$(basename $dataset).perf.wfa.avx512.txt -- ./WFA2-lib/bin/align_benchmark -a gap-affine2p-wfa --wfa-memory ultralow -i $dataset
done
