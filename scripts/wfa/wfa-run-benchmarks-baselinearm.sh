...
for dataset in $DATASETS_PATH/*.seq; do
    ...
    ./WFA2-lib/bin/align_benchmark -a gap-affine2p-wfa -i $dataset --wfa-memory ultralow -o $RESULTS_PATH/arm-$(basename $dataset).wfa.out > $RESULTS_PATH/arm-$(basename $dataset).wfa.stdout 2> $RESULTS_PATH/arm-$(basename $dataset).wfa.stderr
    # Collecting the performance data
    perf stat -a -M TopDownL1 -e inst_retired,cpu_cycles -o $RESULTS_PATH/arm-$(basename $dataset).perf.wfa.txt -- ./WFA2-lib/bin/align_benchmark -a gap-affine2p-wfa --wfa-memory ultralow -i $dataset
done
