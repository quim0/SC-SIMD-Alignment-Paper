...
for dataset in $DATASETS_PATH/$FILTER.seq; do
    ...
    ./WFA2-lib/bin/align_benchmark -a ksw2-extd2-avx512 -i $dataset -o $RESULTS_PATH/$(basename $dataset).ksw2.avx512.out > $RESULTS_PATH/$(basename $dataset).ksw2.avx512.stdout 2> $RESULTS_PATH/$(basename $dataset).ksw2.avx512.stderr
    # Collecting the performance data
    perf stat -a -e instructions,cycles,cache-misses -M TopdownL1 -o $RESULTS_PATH/$(basename $dataset).perf.ksw2.avx512.txt -- ./WFA2-lib/bin/align_benchmark -a ksw2-extd2-avx512 -i $dataset
done
